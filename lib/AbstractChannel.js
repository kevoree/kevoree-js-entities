...
var AbstractChannel = KevoreeEntity.extend({
	...
	internalSend: function (outputPath, msg, callback) {
		...
		if (this.started) {
			this.log.debug(this.toString(), ' ' + this.name + ' -> ' + msg + ' -> [' + debugPaths.join(', ') + ']');
			this.onSend(outputPath, paths, msg + '', callback);
		} else {
			this.log.debug(this.toString(), ' ' + this.name + ' -> ' + msg + ' -> [' + debugPaths.join(', ') + '] (queued)');
			this.pendings.push({
	...
	localDispatch: function (msg, callback) {
		...
		console.log('----- localDispatch (%s) -----', this.name); // eslint-disable-line
		this.getLocalInputPorts().forEach(function (input) {
			console.log(' -> %s.%s -> "%s"', input.compInstance.name, input.portElem.name, msg); // eslint-disable-line
	...
	/**
	 * Returns an array of local Input (only the local inputs of the channel)
