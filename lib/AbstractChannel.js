...
var AbstractChannel = KevoreeEntity.extend({
  ...
  internalSend: function (outputPath, msg, callback) {
        ...
        if (model) {
          ...
          if (port) {
            ...
            if (comp && comp.started) {
              ...
              paths.push({
                path: inputPath,
                debug: comp.eContainer().name + '.' + comp.name + '.' + port.name
              });
            }
          }
        }
    ...
    if (this.started) {
      this.log.debug(this.toString(), ' ' + this.name + ' -> ' + msg + ' -> [' + paths.map(function (p) { return p.debug; }).join(', ') + ']');
      this.onSend(outputPath, paths.map(function (p) { return p.path; }), msg + '', callback);
    } else {
      this.log.debug(this.toString(), ' ' + this.name + ' -> ' + msg + ' -> [' + paths.map(function (p) { return p.debug; }).join(', ') + '] (queued)');
      this.pendings.push({ outputPath: outputPath, content: msg, callback: callback });
    }
  },
